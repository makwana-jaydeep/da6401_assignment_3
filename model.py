...
class EncoderLayer(nn.Module):
    ...
    Post-LayerNorm is used as in the original paper. The LayerNorm is
    applied after the residual addition, which matches equation (1) in
    "Attention Is All You Need". Pre-LayerNorm can improve training
    stability but was not used in the base paper.
...
class Transformer(nn.Module):
    ...
    def __init__(
        self,
        src_vocab_size: int = 10000,  # <-- Added default value
        tgt_vocab_size: int = 10000,  # <-- Added default value
        d_model: int = 512,
        N: int = 6,
        num_heads: int = 8,
        d_ff: int = 2048,
        ...
        # Store spacy/vocab references for use in infer()
        self.src_vocab = None
        self.tgt_vocab = None
    ...
    def infer(self, src_sentence: str) -> str:
        ...
        self.eval()
        assert self.src_vocab is not None, "Set model.src_vocab before calling infer()"
        ...
        with torch.no_grad():
            output = greedy_decode(
                ...
                src_mask,
                max_len=100,
                start_symbol=tgt_sos,
                end_symbol=tgt_eos,
                device=self.device,
            )

        output_tokens = output.squeeze(0).tolist()
        tgt_itos = self.tgt_vocab.get_itos()
        words = []
